...
export default function SuperadminMembersPage() {
  ...
  // Calculate the number of members in each club
  const getClubMemberCounts = (): Record<string, number> => {
    const counts: Record<string, number> = {};
    
    // Initialize counts for all club types
    clubTypes.forEach(club => {
      if (club !== 'ALL') {
        counts[club] = 0;
      }
    });
    ...
    users.forEach(user => {
      if (user.club && user.club !== 'ALL') {
        counts[user.club] = (counts[user.club] || 0) + 1;
      }
    });
  ...
  const clubMemberCounts = getClubMemberCounts();
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-gray-100 py-10 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
        ...
        {/* Club member count cards */}
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 mb-8">
          {Object.entries(clubMemberCounts).map(([club, count]) => (
            <div 
              key={club}
              className={`p-4 rounded-lg shadow-md border ${
                club === 'IEEE' ? 'bg-blue-900/30 border-blue-700' :
                club === 'ACM' ? 'bg-green-900/30 border-green-700' :
                club === 'AWS' ? 'bg-yellow-900/30 border-yellow-700' :
                club === 'GDG' ? 'bg-red-900/30 border-red-700' :
                club === 'STIC' ? 'bg-purple-900/30 border-purple-700' :
                'bg-gray-800 border-gray-700'
              }`}
            >
              <h3 className="text-lg font-bold mb-2">{club}</h3>
              <div className="flex items-center">
                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 mr-2 text-gray-300" viewBox="0 0 20 20" fill="currentColor">
