...
export default function AdminMeetingsPage() {
  ...
  // Filter and sort meetings by date
  const filteredMeetings = meetings
    .filter(meeting => {
      if (activeFilter === 'upcoming') return isFuture(meeting.date);
      if (activeFilter === 'past') return !isFuture(meeting.date);
  ...
  const upcomingCount = meetings.filter(meeting => isFuture(meeting.date)).length;
  const pastCount = meetings.filter(meeting => !isFuture(meeting.date)).length;
          ...
          <div className="bg-gray-800 rounded-lg p-6 shadow-lg border border-gray-700">
            ...
            {filteredMeetings.length === 0 ? (
              <div className="p-8 bg-gray-700 rounded-md text-center">
                ...
                <p className="text-gray-300">No meetings found.</p>
                <p className="text-gray-400 text-sm mt-1">Use the form to schedule your first meeting.</p>
              </div>
            ) : (
