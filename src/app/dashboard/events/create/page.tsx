  ...
  const generateAIDescription = async () => {
    try {
      ...
      if (!form.name || !form.club) {
        throw new Error(
          "Please enter event name and select a club to generate a description"
        );
      }
...
${
  form.startDate
    ? `Start Date: ${new Date(form.startDate).toLocaleString()}`
    : ""
}
...
The description should be engaging, informative, and highlight what attendees can expect from the event. Make it around 3-4 paragraphs.`;
            ...
            messages: [
              {
                role: "system",
                content:
                  "You are an experienced event coordinator for tech events in a college. Write engaging and professional event descriptions.",
              },
              {
                role: "user",
                content: prompt,
  ...
  const getClubHelpText = () => {
    if (userRole === "superadmin")
      return "As a superadmin, you can create events for any club.";
    if (userClub)
      return `Events you create will be associated with ${userClub}.`;
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <div className="bg-gray-800 rounded-xl shadow-md p-8 border border-gray-700">
          ...
          <div className="mt-6">
            ...
            <p className="mt-1 text-sm text-gray-400 ml-8">
              If checked, participants will need approval before they can join
              the event
            </p>
          </div>
        </div>
        ...
        <div className="bg-gray-800 rounded-xl shadow-md p-8 border border-gray-700">
          ...
          <div>
            <label
              htmlFor="description"
              className="block text-sm font-medium text-gray-300 mb-1 flex justify-between items-center"
            >
            ...
            />
            <p className="mt-1 text-sm text-gray-400">
              Fill in all event details above for better AI-generated
              descriptions
            </p>
          </div>
        </div>
