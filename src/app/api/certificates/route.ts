...
export async function POST(request: Request) {
  try {
    ...
    if (!name || !file) {
      return NextResponse.json({ error: 'Certificate name and file are required' }, { status: 400 });
    }
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    if (!userId || !certificateId || approved === undefined || credits === undefined) {
      return NextResponse.json(
        { error: 'User ID, certificate ID, approval status, and credits are required' },
        { status: 400 }
      );
    }
    ...
    // Find certificate in user's certificates array
