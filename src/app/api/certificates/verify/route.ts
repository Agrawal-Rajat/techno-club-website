...
export async function POST(request: Request) {
  try {
    ...
    const adminUser = await User.findOne({ email: session.user.email }, "role");
    ...
    if (
      ...
      isNaN(parseInt(creditsAwarded)) ||
      parseInt(creditsAwarded) < 0
    ) {
      return NextResponse.json(
    ...
    if (user.certificates[certificateIndex].isVerified) {
      return NextResponse.json(
        { error: "Certificate already verified" },
        { status: 400 }
      );
    }
    ...
    user.creditScore = (user.creditScore || 0) + creditsToAward;
