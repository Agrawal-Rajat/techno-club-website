...
export async function POST(request: NextRequest) {
  try {
    ...
    const targetClub =
      currentUser.role === "superadmin" && body.club
        ? body.club
        : currentUser.club;

    if (!targetClub) {
      return NextResponse.json(
        { error: "Club information is required" },
        { status: 400 }
      );
    }
    ...
    if (clubMembers.length === 0) {
      return NextResponse.json(
        { error: "No members found for this club" },
        { status: 404 }
      );
    }

    // Add club members as invitees
    meeting.invitees = clubMembers.map(
      (member) => member._id as mongoose.Types.ObjectId
    );
        ...
        return NextResponse.json(
          {
            meeting: meeting,
            message:
              "Meeting scheduled successfully but email notifications failed",
          },
          { status: 201 }
        );
      }
