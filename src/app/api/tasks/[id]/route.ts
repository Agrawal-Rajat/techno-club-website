  ...
  try {
    ...
    if (userRole === "member") {
      ...
      if (typeof task.assignedTo === "object" && task.assignedTo !== null) {
        assignedToId =
          (
            task.assignedTo as { _id?: mongoose.Types.ObjectId }
          )._id?.toString() || "";
      } else {
    ...
    console.error(`GET /api/tasks - Server error:`, err);
...
export async function PATCH(req: NextRequest, context: Params) {
  try {
    ...
    if (data.status) {
      ...
      // Admin can only update tasks in their club
      if (userRole === "admin") {
        const taskClub = task.club?.toString() || "";
        if (taskClub !== userClub) {
          console.log(
    ...
    // Handle task verification and credit assignment
    if (
      data.isVerified &&
      !task.isVerified &&
    ...
    if (["admin", "superadmin"].includes(userRole)) {
      ...
      // Update isVerified separately for the case where verification is going from true to false
...
export async function DELETE(req: NextRequest, context: Params) {
  try {
    ...
    const isClubAdmin =
      userRole === "admin" && (task.club?.toString() || "") === userClub;
    ...
    const err = error as Error;
    console.error(`DELETE /api/tasks - Server error:`, err);
    console.error(`DELETE /api/tasks - Error stack:`, err.stack);
    return NextResponse.json(
      { error: "Server error", message: err.message },
      { status: 500 }
    );
