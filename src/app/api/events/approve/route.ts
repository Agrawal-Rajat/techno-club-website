  ...
  try {
    ...
    if (user.role !== "superadmin") {
      return NextResponse.json(
        { error: "Unauthorized, only superadmins can approve events" },
        { status: 403 }
      );
    }
    ...
    // Check if user already approved this event - Fix: Properly type user._id
    const userIdStr = (user._id as mongoose.Types.ObjectId).toString();
    const alreadyApproved = event.approvals?.some(
